...
def test_validator_detects_placeholder_mismatch(tmp_path: Path) -> None:
    ...
    en = {
        ...
        "messages": {"user": {"greeting": "Hello {username}"}},
        "enums": {},
        "faqs": {},
    }
...
def test_validator_rejects_nested_placeholder_paths(sample_i18n_files: tuple[Path, Path]) -> None:
    locales_dir, manifest_path = sample_i18n_files
    ...
    with pytest.raises(PlaceholderError):
        validator.validate_single_locale_data("en", data)
...
def test_editor_delete_missing_path_raises(sample_i18n_files: tuple[Path, Path]) -> None:
    ...
    with pytest.raises(LocaleEditError):
        editor.delete_value("fa", "messages.user.not_here")


def test_formatter_timezone_and_renderer_pipeline() -> None:
    ...
    assert formatter.format_datetime(value, locale="en") == "en:2026/04/15 04:30:00"
