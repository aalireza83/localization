...
class LocaleRepository:
    ...
    def _load_manifest(self) -> ManifestData:
        ...
        default_locale = default_locale.strip().lower()
        ...
        for locale_code, descriptor in locales.items():
            ...
            normalized_code = locale_code.strip().lower()
            if normalized_code in normalized_locales:
                raise ManifestError(f"Duplicate locale code in manifest: {normalized_code}")
            ...
            normalized_locales[normalized_code] = descriptor

        if default_locale not in normalized_locales:
            raise ManifestError("Manifest 'default_locale' must reference a locale declared in 'locales'.")
