...
class LocaleRenderer(Protocol):
    """Render normalized temporal values into final localized strings."""
...
class LocaleValueFormatter:
    ...
    Stage 1 (datetime values): timezone normalization.
    Stage 2 (date/datetime values): locale rendering to final string.
    ...
    Naive datetime rule:
    - If ``naive_input_timezone`` is not set, naive datetimes are kept unchanged.
    - If set, naive datetimes are interpreted in ``naive_input_timezone`` and then normalized.
    """
