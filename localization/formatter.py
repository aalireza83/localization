...
_BUILTIN_RENDERER = StrftimeRenderer()
...
class LocaleValueFormatter:
    """Formats wrapped values using a two-stage temporal pipeline.

    Stage 1: timezone normalization for datetime values.
    Stage 2: locale renderer produces final string.
    ...
    - If naive_input_timezone is None (default), naive datetimes are left untouched.
    - If naive_input_timezone is set, naive datetimes are interpreted in that timezone
      before normalizing to the resolved target timezone.
    """
    ...
    def __post_init__(self) -> None:
        ...
        self._resolved_default_renderer = self.default_renderer or _BUILTIN_RENDERER
